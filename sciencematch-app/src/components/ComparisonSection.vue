...
.floating-frame:hover {
  opacity: 1;
  box-shadow:
    0 30px 60px rgba(153, 0, 27, 0.12),
...
.frame-4 {
  ...
  right: 30px;
  transform: rotate(-3deg);
  animation: float-4 9.5s ease-in-out infinite;
}

/* Float animations - plus subtiles */
@keyframes float-1 {
  0%, 100% {
    transform: rotate(-4deg) translateY(0) translateX(0);
  }
  50% {
    transform: rotate(-4deg) translateY(-8px) translateX(3px);
  }
}
...
.table-header .brand-col {
  color: #FF6B7A;
  text-shadow: 0 0 20px rgba(255, 107, 122, 0.3);
}
